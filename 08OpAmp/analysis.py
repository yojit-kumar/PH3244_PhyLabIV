...
Vin_AC_diff = 1.0       # Volts (AC RMS)
C_diff_One = 0.1e-6     # Farads (0.1 uF)
R2_actual_diff_One = 100000.0  # Ohms (100 kOhms)
C_diff_Two = 0.05e-6    # Farads (0.05 uF)
R2_actual_diff_Two = 500000.0  # Ohms (500 kOhms)
...
def process_amplifier(file_path, set_name, is_inverting, R_in, R_f):
    ...
    prefix = "inv" if is_inverting else "non"
    ...
    save_latex_table(df, f"Observation table for {prefix} amplifier (Set {set_name}).", 
                     f"tab:{prefix}_obs_{set_name}", OBS_TABLES_FILE, float_fmt="%.2f")
...
def process_frequency_circuit(file_path, set_name, is_integrator, Vin, Known_Comp, Actual_Comp):
    ...
    if is_integrator:
        # slope = 2 * pi * R1 * C  => C = slope / (2 * pi * R1)
        calc_val = m_exp / (2 * np.pi * Known_Comp)
        prefix = "int"
    else:
        # slope = 2 * pi * R2 * C  => R2 = slope / (2 * pi * C)
        calc_val = m_exp / (2 * np.pi * Known_Comp)
        prefix = "diff"
    ...
    # Save Observation Table
    save_latex_table(df, f"Observation table for {prefix} circuit (Set {set_name}).", 
                     f"tab:{prefix}_obs_{set_name}", OBS_TABLES_FILE, float_fmt="%.4f")
...
# 4. Differentiator
df_diff1, y_diff1, m_diff1, ylabel_diff = process_frequency_circuit("data/diff_1.csv", "One", False, Vin_AC_diff, C_diff_One, R2_actual_diff_One)
df_diff2, y_diff2, m_diff2, _ = process_frequency_circuit("data/diff_2.csv", "Two", False, Vin_AC_diff, C_diff_Two, R2_actual_diff_Two)

plt.figure(figsize=(8, 5))
plt.scatter(df_diff1['Frequency'], y_diff1, label='Set 1 Data', color='blue')
plt.plot(df_diff1['Frequency'], linear_fit_origin(df_diff1['Frequency'], m_diff1), 'b--', label=f'Set 1 Fit (slope={m_diff1:.2e})')
plt.scatter(df_diff2['Frequency'], y_diff2, label='Set 2 Data', color='red')
