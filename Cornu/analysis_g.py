...
# ---------------------------------------------------------
# FILL IN YOUR BEAM CONSTANTS HERE (in meters):
# ---------------------------------------------------------
L = 0.0  # Distance between the two triangular pegs
w = 0.0  # Width of the transparent beam
h = 0.0  # Thickness (height) of the transparent beam
# ---------------------------------------------------------
...
weight4 = pd.read_csv("data/weight_300.csv") 

# Actual weight values used for tables and calculations
weightArray = [0.0, 100.0, 196.5, 249.7, 300.0]
masses_g = [100.0, 196.5, 249.7, 300.0] 
weights_data = [weight1, weight2, weight3, weight4]
...
# --- R0 ANALYSIS (Restored from your original code) ---
...
save_latex_cmd('RZeroTwo', R02) # Corrected variable name from your original code
...
fig_R1, axs_R1 = plt.subplots(2, 2, figsize=(12, 10))
fig_R2, axs_R2 = plt.subplots(2, 2, figsize=(12, 10))
axs_R1 = axs_R1.flatten()
axs_R2 = axs_R2.flatten()
...
for i, (wt, mass) in enumerate(zip(weights_data, masses_g)):
    ...
    # Get the number of rows (rings) for this specific weight to prevent NaN mismatch
    N = len(wt)
    x_valid = 1.0 / np.arange(1, N + 1)
    
    # Calculate diameters for the bent beam (ensure we only take N rows as numpy arrays)
    ...
    axs_R1[i].plot(x_valid, linear_fit_origin(x_valid, m1), 'k--', label='Fit')
    ...
    axs_R2[i].plot(x_valid, linear_fit_origin(x_valid, m2), 'k--', label='Fit')
